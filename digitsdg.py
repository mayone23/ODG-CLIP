...
label_dom1=[]
class_names1=[]
path_dom1='./data/digits_dg/'+domains[0]+'/train'
...
label_dom2=[]
class_names2=[]
path_dom2='./data/digits_dg/'+domains[1]+'/train'
...
label_dom3=[]
class_names3=[]
path_dom3='./data/digits_dg/'+domains[2]+'/train'
...
class_names.sort()
train_prev_classnames = class_names[:7]
...
def train_epoch(model, unknown_image_generator, domainnames, train_loader, optimizer, lr_scheduler, step):
    ...
    for img_prev, domain_prev, label_prev, label_one_hot_prev in tqdm_object:
        ...
        unknown_posprompt = "a photo of an unknown digit"
...
test_label_dom=[]
test_domain_names=[]
test_path_dom='./data/digits_dg/'+domains[3]+'/val'
...
test_label_class_final_modified = [label if label <= 6 else 7 for label in test_label_class_dom]
...
best_open_set_acc = 0
best_avg_acc = 0
accuracy_file_path = "./results/digits_dg/syn.txt"  
...
for epoch in range(num_epochs):
    ...
    TRAIN_MODEL_PATH = Path("./train_models/digits_dg/syn")
    TRAIN_MODEL_PATH.mkdir(parents=True, exist_ok=True)
    TRAIN_MODEL_NAME = f"syn_{epoch+1}.pth"
    ...
    MODEL_PATH = "./train_models/digits_dg/syn"
    MODEL_NAME = f"syn_{epoch+1}.pth"
    ...
    with torch.no_grad():
        ...
        for test_img, test_domain, test_label, test_label_one_hot in test_tqdm_object:
            ...
            class_a_mask = (test_label <= 6) 
            class_b_mask = (test_label > 6)
        ...
        if average_acc > best_avg_acc:
            ...
            TEST_MODEL_PATH = Path("./test_models/digits_dg")
            TEST_MODEL_PATH.mkdir(parents=True, exist_ok=True)
            TEST_MODEL_NAME = "syn.pth"
